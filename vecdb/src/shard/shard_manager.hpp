...
public:
    ...
    // Phase 7: direct index access for HTTP graph inspector
    HNSWIndex* get_shard_index(uint32_t shard_id) {
        if (shard_id >= shard_count_) return nullptr;
        return shards_[shard_id]->index.get();
    }
